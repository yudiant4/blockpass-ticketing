...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (!event) return res.status(404).json({ status: 'error', message: 'Acara tidak ditemukan' });
...
router.post('/', requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const totalQuota = tiers.reduce((sum: number, t: any) => sum + (t.quota || 0), 0);
    if (totalQuota < 1) {
      return res.status(400).json({ status: 'error', message: 'Total quota minimal 1' });
    }

    const validTiers = tiers.map((t: any) => ({
...
router.post('/:id/buy-intent', async (req: Request, res: Response) => {
  try {
    const { tierId, wallet, quantity = 1 } = req.body;
    if (!tierId || !wallet) return res.status(400).json({ status: 'error', message: 'tierId & wallet wajib' });

    const event = await Event.findById(req.params.id);
    if (!event) return res.status(404).json({ status: 'error', message: 'Acara tidak ditemukan' });

    const tierIndex = event.tiers.findIndex(t => t.tierId === tierId);
    if (tierIndex === -1) return res.status(404).json({ status: 'error', message: 'Tier tidak ditemukan' });

    const tier = event.tiers[tierIndex];
    const available = tier.quota - tier.sold;
    if (available < quantity) return res.status(400).json({ status: 'error', message: `Kuota tidak cukup (tersisa ${available})` });
    if (quantity > tier.maxPerWallet) return res.status(400).json({ status: 'error', message: `Maksimal ${tier.maxPerWallet} per wallet` });
