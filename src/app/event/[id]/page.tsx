  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 py-24">
        ...
        {/* Tab Navigation */}
        <div className="flex justify-center gap-3 mb-8 p-2 bg-slate-900/80 border border-cyan-500/40 rounded-xl backdrop-blur-xl">
          {[
            ...
            <button
              ...
              className={`flex-1 px-4 py-2.5 rounded-lg font-mono text-xs font-bold uppercase tracking-widest transition-all duration-200 ${
                activeTab === tab.key
                  ? 'bg-cyan-500 text-slate-950 shadow-xl shadow-cyan-500/30 scale-100'
                  : 'bg-slate-950/60 text-slate-400 border border-cyan-500/20 hover:text-cyan-400 hover:border-cyan-500/60 hover:scale-102'
              }`}
            >
              {tab.label}
            </button>
        ...
        {/* Tab Content */}
        {activeTab === 'pre' && (
          <div className="bg-slate-900/60 border border-cyan-500/30 rounded-2xl p-6 backdrop-blur-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              {ticketTiers.map((tier) => (
                <button
                  ...
                  className={`relative p-4 rounded-xl text-left transition-all duration-300 bg-slate-900/60 backdrop-blur-xl overflow-hidden ${
                    selectedTier === tier.id
                      ? `${tier.border} ${tier.glow} ring-2 ring-cyan-500/50 scale-[1.02]`
                      : 'border border-cyan-500/20 hover:border-cyan-500/60 hover:scale-102'
                  }`}
                >
                  {/* Tier accent bar */}
            ...
            <button className="w-full py-3 px-6 bg-cyan-500 text-slate-950 font-mono text-xs font-bold uppercase tracking-wider rounded-lg hover:bg-cyan-400 transition-all duration-200 shadow-xl shadow-cyan-500/30 hover:shadow-2xl hover:shadow-cyan-500/40 hover:scale-[1.01] active:scale-[0.99]">
              {'>'} MINT TIKET SEKARANG
            </button>
        ...
        {activeTab === 'day' && (
          <EventDayTicket
            ...
            tierData={ticketTiers.find((t) => t.id === selectedTier) || ticketTiers[1]}
        ...
        {activeTab === 'post' && (
          <div className="bg-slate-900/60 border border-emerald-500/30 rounded-2xl p-8 text-center backdrop-blur-xl">
        ...
        <div className="mt-10 text-center">
          <Link
            href="/marketplace"
            className="inline-block font-mono text-xs uppercase tracking-widest text-slate-500 hover:text-cyan-400 transition-colors border border-slate-700 hover:border-cyan-500/40 px-4 py-2 rounded hover:bg-slate-900/60 backdrop-blur"
          >
